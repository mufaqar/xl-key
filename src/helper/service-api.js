...
export default class ServiceApi {
    ...
    getAllServices = () => {
        return this.init().get("services/all");
    }

    createService = (data) => {
        return this.init().post("services/create", data);
    };

    updateService = (data) => {
        let url = "services/update/" + data[0];
        console.log("url ***", url);
        return this.init().put(url, data[1]);
    };

    orderUpService = (id) => {
        let url = "services/order-up/" + id;
        return this.init().put(url);
    };

    orderDownService = (id) => {
        let url = "services/order-down/" + id;
        return this.init().put(url);
    };

    deleteService = (data) => {
        let url = "services/delete/" + data;
        console.log("url ***", url);
        return this.init().delete(url);
    };

    getAllAdminServices = () => {
        return this.init().get("services/all/admin");
    }

    getAllParentServices = () => {
        return this.init().get("services/all/parent");
    }
