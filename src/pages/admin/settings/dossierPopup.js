...
class Dossiermodal extends React.Component {
    ...
    updateAccount() {
        ...
        this.state.api
            .updateFolder([data.id, dataObject])
            .then(response => {
                    console.log("data  new ", response);
                    this.setState({
                        message: response.data.message
                        ...
                        <div class="modal-header">
                            {this.props.type == "update" && (
                                <h5 className="modal-title textColor" id="exampleModalLabel">
                                    {translation.Update_a_note_folder}
                                </h5>
                            )}
                            {this.props.type == "create" && (
                                <h5 className="modal-title textColor" id="exampleModalLabel">{translation.Add_note_folder}</h5>
                            )}
...
export default connect(mapStateToProps, null)(withLanguageHook(Dossiermodal));