...
const Modal = (props) => {
    ...
    const [api, setApi] = useState(new Api(props.token, props.acc_id,  props.access_level, props.access_key));
    ...
    const getNextPhoto = () => {
        let new_index = index;
        if (props.page != 1) {
            new_index = 12 * (props.page - 1) + (index + 1);
        } else {
            new_index = (index + 1);
        }


        if (props.total < new_index) new_index = props.total;
        if (0 > new_index) new_index = 1;
  
        setClicked(true, setIndex(new_index, getSelectedPhoto(new_index)));
    }
    ...
    const getSelectedPhoto = (new_index) => {
        ...
        api
            .getSelectedPhoto([props.limit, props.page, props.sites, props.years,
            props.tags, props.categories, props.passages, props.archieveResult, props.favourite, new_index])
            .then(response => {
                if (response.status === 200) {

                    console.log("response photo ",response.data.data[0] );
                    setPhoto(response.data.data[0])
                    getComments(response.data.data[0].id);
                }
            })
            .catch(err => {
                if (err.response && err.response.status == 401) {
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-12">
                                            <div className="iconPadding">
                                                ...
                                                <NotePopUp id={"modalDelete"} imageUrl={photo.url}></NotePopUp>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                                                        ...
                                                                        <Popover
                                                                            ...
                                                                            content={() => ( // you can also provide a render function that injects some useful stuff!
                                                                                <div className="popover_style">
                                                                                    ...
                                                                                    <strong className="poppver_text_style" htmlFor="inputEmail">{translation.Delete}</strong>
                                                                                </div>
