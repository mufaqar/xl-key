...
import Photos from "../../../public/img/Photos.svg";
...
function ProducerMainNavigation(props) {
    const dispatch = useDispatch();
    const user_fullname = useSelector(state => state.name);
    const token = useSelector(state => state.token);
    const user_account_logo = useSelector(state => state.logo);
    const [logo, setLogo] = React.useState(user_account_logo);
    const [userName, setUserName] = React.useState("Producer");
    const [api, setApi] = React.useState(new Api(token, props.acc_id, props.access_level, props.access_key));
    const [access_level, setAccess_level] = React.useState(props.access_level);
    const [notificationItem, setNotificationItem] = React.useState([]);

    if (props.acc_id) getHeader();
    if (notificationItem == null || notificationItem.length == 0) getNotification();
    ...
    function getHeader() {
        api.getHeader(props.acc_id)
            ...
            .then(response => {
                ...
                if (language == "FR") {
                    setUserName(data.name_fr);
                } else {
                    setUserName(data.name_en);
                }
                if (data.logo_url) {
    ...
    const navigateToHome = () => {
        ...
        } else if (base_url.includes("entreprise")) {
            homeUrl = "/entreprise/dashboard";
        } else if (base_url.includes("consultant")) {
            homeUrl = "/consultant/dashboard";
        }
    ...
    return (
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            ...
            {!show ? <Popup anchor={anchor.current} show={!show} popupClass={'popup-content'}>
                ...
                <div className="row">
                    <div className="col-5 ml-5">
                        ...
                        <Link to={base_url + "/photos"}>
                            <div className="row">
                                <img src={Photos} width="40" height="40" style={{ marginTop: '8px' }} />
                                <div className="column">
                                    <div className="topic-font ml-0">{translation.Photos}</div>
                                    <div className="description-font ml-0"></div>
                                </div>
                            </div>
                        </Link>
                        {!(access_level == 1 || access_level == 2) && (
                            <Link to={base_url + "/notes"}>
                                <div className="row">
                                    <img src={Notes} width="40" height="40" style={{ marginTop: '8px' }} />
                                    <div className="column">
                                        <div className="topic-font ml-0">{translation.Notes}</div>
                                        <div className="description-font ml-0"></div>
                                    </div>
                                </div>
                            </Link>
                    ...
                    <div className="col-5 ml-5">
                        ...
                        {!(access_level == 1 || access_level == 2) && (
                            <Link to={base_url + "/services"}>
                                <div className="row">
                                    <img src={Services} width="40" height="40" style={{ marginTop: '8px' }} />
                                    <div className="column">
                                        <div className="topic-font ml-0">{translation.Services}</div>
                                        <div className="description-font ml-0"></div>
                                    </div>
                                </div>
                            </Link>
                        ...
                        {!(access_level == 1 || access_level == 2) && (
                            <Link to={base_url + "/settings/information-general"}>
                                <div className="row">
                        ...
                        <Link to={base_url + "/notifications"}>
                            <div className="row">
                                <img src={Notification} width="40" height="40" style={{ marginTop: '8px' }} />
                                <div className="column">
                                    <div className="topic-font ml-0">{translation.Notifications}</div>
                                    <div className="description-font ml-0"></div>
                                </div>
                            </div>
                        </Link>
            ...
            <a class="navbar-brand logoMargin" style={{ width: "25%" }} href={base_url + "/dashboard"}><img src={logo} height="35" style={{ height: '75px' }} /></a>
            {props.acc_id != null && (
                <p className="alert-warning pt-3 pb-3 pl-4 pr-4">
                    <span>{translation.Currently_you_are_in_the} {userName} ({translation.Producer})</span>
                    <button className="btn-primary ml-2 pl-2 pr-2 pt-1 pb-1 text-center" onClick={navigateToHome}>
                        {translation.Go_Back}
                    </button>
                </p>
            )}

            <a class="navbar-brand bellIconMoble " href={base_url + "/notifications"}>
                ...
                <span className="notificationDot" >
                    {(notificationItem != null && notificationItem.length != 0 &&
                        (getStatus(notificationItem[0].queue))) && (<svg
            ...
            <div class="collapse navbar-collapse flex-grow-1 text-right" id="navbarSupportedContent1">
                <ul class="navbar-nav ml-auto flex-nowrap">
                    <Link to={base_url + "/notifications"}>
                        <li class="nav-item belPadding belStyle">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd" clip-rule="evenodd"
                                    d="M12 2C10.3725 2 9.04834 3.19359 9.04834 4.66055V5.46328H10.5751V4.66055C10.5751 3.95231 11.2145 3.37613 12 3.37613C12.7857 3.37613 13.4252 3.95227 13.4252 4.66055V5.46328H14.9519V4.66055C14.9519 3.19359 13.6275 2 12 2Z"
                                    fill="black" />
...
export default ProducerMainNavigation;