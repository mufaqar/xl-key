...
import FieldPopUp from "./field-popup";
...
class Field extends React.Component {
    ...
    toggleFieldPopover = (index, prevPropCatId) => {
        let propCatId = index;
        if (prevPropCatId == index) {
            propCatId = null;
        }
        this.setState((prevState) => ({mobileFieldPopoverId: propCatId}));
    }
    ...
    render() {
        const {isHidden, fields} = this.state;
        const translation = this.props.translation;
        return (
            <div>
                {/* <FieldPopUp id={"modalAddField"+ this.state.siteId} data={{}} type={"create"}
                            title={"Ajouter un field"} fieldId="" siteId={this.state.siteId}>
                </FieldPopUp> */}
                <div>
                    <button
                        ...
                        <span className="app__toggle-text"
                              style={{fontWeight: 'bold'}}>Nom des champs/Field Names</span>
                                ...
                                <div className="popover_style">
                                    {this.state.type !== "PRODUCTEUR" && (
                                        <div data-toggle="modal"
                                             data-target={"#addFields"+this.state.siteId}>
                                            <img src={addIcon} className="img-responsive"
                                                 alt="Responsive image"/>
                                            <strong className="poppver_text_style"
                                                    htmlFor="inputEmail">{translation.Add_a_field}</strong>
                                        </div>
                                    )}
                                </div>
                            )}>
                        ...
                        <FieldPopUp id={"addFields" +this.state.siteId} data={{}} type={"create"}
                                    title={"Ajouter un field"} fieldId="" siteId={this.state.siteId}>
                        </FieldPopUp>
                    </button>
                        ...
                        {fields != null && (fields).map((field) => (
                            ...
                            <div className="admin-column-white">
                                <div className="row ">
                                    <div className="col-7">
                                        <label
                                            className="ml-4 mt-2 mb-3">{field.name_fr} / {field.name_en}</label>
                                    </div>
                                    <div className="col-5 align-center pl-0 pr-0">
                                        <div className="text-right">
                                            <div>
                                                ...
                                                <img src={editIcon}
                                                     data-toggle="modal"
                                                     data-target={"#modalField" + field.id}
                                                     className="img-responsive image d-none d-sm-inline-block mt-0 mb-0"
                                                     alt="Responsive "/>
                                                <img src={closeIcon}
                                                     className="img-responsive image d-none d-sm-inline-block mt-0 mb-0"
                                                     alt="Responsive "
                                                     data-toggle="modal"
                                                     data-target={"#deleteField" + field.id}/>
                                                <div>
                                                    <Popover

                                                        isOpen={this.state.mobileFieldPopoverId == field.id ? true : false}
                                                        position={'bottom'}
                                                    ...
                                                    >
                                                        <img src={glyphIvon}
                                                             ...
                                                             onClick={() => this.toggleFieldPopover(field.id, this.state.mobileFieldPopoverId)}
                                                             alt="Responsive "/>
                                                    </Popover>
                                                </div>
                                            </div>
                                            <div class="modal fade" id={"deleteField" + field.id} tabindex="-1"
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-primary"
                                                                    onClick={this.deleteField.bind(null, field.id)}
                                                                    style={{width: '40%'}}>
                                                                {translation.Yes}
                                                            </button>
                                            ...
                                            <FieldPopUp id={"modalField" + field.id} data={field}
                                                        type={"update"} title={"Renommer la field"}
                                                        fieldId={field.id} siteId={this.state.siteId}>
                                            </FieldPopUp>
                                        </div>
                                    </div>
                                </div>
...
export default connect(mapStateToProps, null)(withLanguageHook(Field));